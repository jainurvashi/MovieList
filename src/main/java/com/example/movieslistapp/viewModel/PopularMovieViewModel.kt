...
class PopularMovieViewModel constructor(private val mainRepository: MainRepository) : ViewModel() {
    ...
    fun getAllPopularMovies(pageNumber:Int) {
        Log.d("Thread Outside", Thread.currentThread().name)

        viewModelScope.launch {
            Log.d("Thread Inside", Thread.currentThread().name)
            when (val response = mainRepository.getAllPopularMovies(pageNumber)) {
